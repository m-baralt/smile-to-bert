...
class SMILESDataset(Dataset):
    ...
    def __getitem__(self, item):
        file = f'data_spe/data_tensors/tensor_files/{os.path.basename(str(self.files_index[item][0], encoding="utf-8"))}'
...
# Load data
properties_tensor = torch.load('data_spe/data_tensors/filt_props_tensor.pt').numpy()
ifile = np.load("data_spe/data_tensors/file_index.npy")
...
properties_tensor = torch.load('data_spe/data_tensors/filt_props_tensor.pt')
...
if __name__ == "__main__":
    training_loop(tokenizer = smiles_tokenizer, files_index = ifile,
                  ...
                  n_layers = 4, heads = 6, dropout = 0.1, seq_length = 113, 
                  num_properties = 7, num_workers = 10,
                  batch_size = 64, lr = 1e-4, betas = (0.9, 0.999), 
